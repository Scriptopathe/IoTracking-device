...
struct reference { double north=43.574848; double south=43.560980; double east=1.486931; double west=1.452127; }; reference fixref;
struct coords transformCoords(struct rawCoords rc) {
    ...
    CONSOLE_SERIAL.println(String(fixref.west));
    CONSOLE_SERIAL.println(String(fixref.south));
    CONSOLE_SERIAL.println(String(fixref.east));
    CONSOLE_SERIAL.println(String(fixref.north));
    CONSOLE_SERIAL.println(String(rc.latitude - fixref.west));
    CONSOLE_SERIAL.println(String(rc.longitude - fixref.south));
    CONSOLE_SERIAL.println(String(fixref.east - fixref.west));
    CONSOLE_SERIAL.println(String(fixref.north - fixref.south));
    CONSOLE_SERIAL.println(String(((rc.latitude - fixref.west) / (fixref.east - fixref.west))*1023));
    CONSOLE_SERIAL.println(String(((rc.longitude - fixref.south) / (fixref.north - fixref.south))*1023));*/
  return {
     x: (((rc.latitude - fixref.west) / (fixref.east - fixref.west))*1023),
     y: (((rc.longitude - fixref.south) / (fixref.north - fixref.south))*1023)
        };
};
...
{  rawCoords rc_from_gps;
   ...
   if (sodaq_gps.scan(false, timeout)) {
        ...
        rc_from_gps= {(sodaq_gps.getLat()),(sodaq_gps.getLon())};
...
void loop()
{
  ...
  //C'est partie est à vérifier avec Clovis & Josué  
